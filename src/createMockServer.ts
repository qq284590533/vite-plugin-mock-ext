...
export async function createMockServer(
  ...
  if (opt.mockUrl) {
    ...
    if (!regexp.test(opt.mockUrl)) {
      throw '[vite-plugin-mock]: mockUrl 字段必须以 "/" 开头（需满足正则 /^\\/\\b/ ）'
    }
  }
...
async function getMockConfig(opt: ViteMockOptions) {
  ...
  const { absConfigPath, absMockPath } = getPath(opt)
  const { ignore, configPath, logger, mockUrl } = opt
  ...
  try {
    ...
    loggerOutput(`mock reload error`, error as any)
    ret = []
  }
